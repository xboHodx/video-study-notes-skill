...
def _find_repo_root() -> Path | None:
    for parent in Path(__file__).resolve().parents:
        if (parent / "scripts").is_dir() and (parent / "requirements-skill.txt").is_file():
            return parent
    return None
...
def build_parser() -> argparse.ArgumentParser:
    return argparse.ArgumentParser(
        prog="video-notes check",
        description="Check whether the local runtime environment satisfies skill prerequisites.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if nvidia_smi and cublas:
        print_status("OK", "CPU mode ready; GPU mode should be available and verified at runtime.")
    else:
